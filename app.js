...
function _saveToStorageImpl() {
  ...
  _saveTimer = setTimeout(async () => {
    try {
      // Phase 1 migration IndexedDB (v2026.07.13, cf. migration-indexeddb.md) : window._sb
      // pointe désormais systématiquement vers le shim LocalDB (IndexedDB), initialisé une
      // fois au démarrage par initLocalDB() — plus jamais vers le vrai client Supabase pour
      // les opérations courantes. saveToSupabase() (nom conservé, corps INCHANGÉ) écrit donc
      // maintenant dans IndexedDB, gratuitement et sans limite de quota pratique — d'où la
      // disparition de l'ancien double-cache localStorage (writeLocalCache(), neutralisée
      // plus haut) qui n'a plus de raison d'être.
      if (window._sb) {
        await saveToSupabase();
      } else {
        // Ne devrait plus arriver en usage normal (initLocalDB() tourne avant tout le reste),
        // gardé par sécurité si saveToStorage() est appelée avant la fin du boot.
...
function writeLocalCache() {
  // Neutralisé (v2026.07.13, Phase 1 migration IndexedDB) : LocalDB (le shim window._sb,
  // cf. section INDEXEDDB plus haut) persiste déjà chaque table dans IndexedDB à chaque
  // écriture — ce cache localStorage (limité à 5-10 Mo, cause historique des bugs "quota
  // dépassé" ci-dessous) redeviendrait exactement le goulot d'étranglement que la migration
  // IndexedDB cherche à éliminer. Fonction conservée vide (plutôt que supprimée) pour ne pas
  // avoir à toucher ses ~3 points d'appel — nettoyage complet prévu en Phase 3.
  return;
...
function writeLastfmTracksCache() {
  // Neutralisé (v2026.07.13, Phase 1 migration IndexedDB) — même raison que writeLocalCache()
  // ci-dessus : LocalDB persiste déjà lastfm_tracks dans IndexedDB, ce cache localStorage
  // compact (dernier rempart avant le correctif v2026.07.12-15) n'a plus lieu d'être.
  return;
  try {
...
// ===================== INDEXEDDB — source de vérité locale (Phase 1, v2026.07.13) =====================
// cf. migration-indexeddb.md. Stratégie retenue : plutôt que de réécrire un par un les ~86
// appels `window._sb.from(...)` répartis dans ce fichier (albums/tracks/associations/
// lastfm_data/rym_data/meta/sync_state/album_tracks/musicbee_tracks/collection_snapshots),
// on remplace ce qu'il y a DERRIÈRE `window._sb` : LocalDB ci-dessous réimplémente le sous-
// ensemble de l'API supabase-js réellement utilisé ici (from().select/insert/update/upsert/
// delete/eq/neq/in/range/order/single, thenable), mais persiste dans IndexedDB au lieu du
// réseau. Résultat : saveToSupabase(), loadFromSupabase(), loadLastfmFromSupabase(),
// loadAlbumTracks(), loadMusicBeeTracks(), createSnapshot() etc. fonctionnent TOUS sans
// modification — ils croient toujours parler à Supabase, mais parlent en réalité à IndexedDB
// (gratuit, quota large, zéro egress). Ce choix limite considérablement le risque de cette
// 1ère passe par rapport à une réécriture point par point. Le vrai client Supabase
// (supabase.createClient) n'est plus utilisé qu'une fois, pour la migration ponctuelle
// (migrateSupabaseToIndexedDB) — jamais plus pour les opérations courantes.
const IDB_NAME = 'discotheque_idb';
const IDB_VERSION = 1;
const IDB_STORE = 'tables'; // un seul object store ; clé = nom de "table", valeur = tableau de lignes

let _idbDbPromise = null;
function idbOpen() {
  if (_idbDbPromise) return _idbDbPromise;
  _idbDbPromise = new Promise((resolve, reject) => {
    if (!window.indexedDB) { reject(new Error('IndexedDB non disponible dans ce navigateur')); return; }
...
// Builder chaînable façon supabase-js — thenable, exécuté paresseusement à l'await/.then().
// Les méthodes de filtre (eq/neq/in) et l'opération (select/insert/update/upsert/delete)
// peuvent être chaînées dans n'importe quel ordre (comme supabase-js réel), puisque rien ne
// s'exécute avant _exec().
class LocalDBQuery {
  constructor(db, table) {
    this._db = db; this._table = table; this._op = 'select';
    this._payload = null; this._onConflict = null; this._filters = [];
    this._range = null; this._order = null; this._limit = null;
    this._single = false; this._cols = null;
  }
  select(cols) {
    // Après insert/update/upsert, .select() ne fait que demander les colonnes en retour
    // (RETURNING), ça ne change pas l'opération — comme en supabase-js réel.
    if (this._op !== 'insert' && this._op !== 'update' && this._op !== 'upsert') this._op = 'select';
  ...
  upsert(rows, opts) { this._op = 'upsert'; this._payload = Array.isArray(rows) ? rows : [rows]; this._onConflict = (opts && opts.onConflict) || 'id'; return this; }
  delete() { this._op = 'delete'; return this; }
  eq(col, val)  { this._filters.push(r => r[col] === val); return this; }
  neq(col, val) { this._filters.push(r => r[col] !== val); return this; }
  in(col, arr)  { const set = new Set(arr); this._filters.push(r => set.has(r[col])); return this; }
  ...
  then(onResolve, onReject) { return this._exec().then(onResolve, onReject); }
  catch(onReject) { return this._exec().catch(onReject); }
  async _exec() {
    try { return await this._db._run(this._table, this); }
...
class LocalDB {
  ...
  _nextId(name) {
    if (!this._nextIds[name]) {
      let max = 0;
      (this._cache[name] || []).forEach(r => { if (typeof r.id === 'number' && r.id > max) max = r.id; });
      this._nextIds[name] = max;
    }
    return ++this._nextIds[name];
  }
  async _run(table, q) {
    const rows = await this._getTable(table);
    const matches = r => q._filters.every(f => f(r));
    ...
    if (q._op === 'select') {
      ...
      if (q._range) result = result.slice(q._range[0], q._range[1] + 1);
      if (q._limit != null) result = result.slice(0, q._limit);
      if (q._cols && q._cols !== '*') {
        const cols = q._cols.split(',').map(c => c.trim());
        result = result.map(r => { const o = {}; cols.forEach(c => o[c] = r[c]); return o; });
      }
      if (q._single) {
        return result.length ? { data: result[0], error: null } : { data: null, error: { code: 'PGRST116', message: 'No rows found' } };
      }
      return { data: result, error: null };
    }

    if (q._op === 'insert') {
      const inserted = q._payload.map(row => {
        const r = { ...row };
        if (r.id === undefined || r.id === null) r.id = this._nextId(table);
        rows.push(r);
        return r;
      });
      await this._persist(table);
      return q._single ? { data: inserted[0] || null, error: null } : { data: inserted, error: null };
    }

    if (q._op === 'update') {
      const updated = [];
      rows.forEach(r => { if (matches(r)) { Object.assign(r, q._payload); updated.push(r); } });
      await this._persist(table);
      if (q._single) return updated.length ? { data: updated[0], error: null } : { data: null, error: { code: 'PGRST116', message: 'No rows found' } };
      return { data: updated, error: null };
    }

    if (q._op === 'upsert') {
      const conflictCols = q._onConflict.split(',').map(c => c.trim());
      const index = new Map();
      rows.forEach((r, i) => index.set(conflictCols.map(c => String(r[c] ?? '')).join('|||'), i));
      const upserted = [];
      q._payload.forEach(row => {
        const key = conflictCols.map(c => String(row[c] ?? '')).join('|||');
        if (index.has(key)) {
          const i = index.get(key);
          rows[i] = { ...rows[i], ...row };
          upserted.push(rows[i]);
        } else {
          const r = { ...row };
          if (r.id === undefined || r.id === null) r.id = this._nextId(table);
          rows.push(r);
      ...
      await this._persist(table);
      return q._single ? { data: upserted[0] || null, error: null } : { data: upserted, error: null };
    }

    if (q._op === 'delete') {
      const remaining = [], deleted = [];
      rows.forEach(r => (matches(r) ? deleted.push(r) : remaining.push(r)));
      this._cache[table] = remaining;
      await this._persist(table);
      return { data: deleted, error: null };
    }

    return { data: null, error: { message: 'Opération LocalDB non supportée: ' + q._op } };
  }
}

// Initialise le shim et vérifie qu'IndexedDB répond — appelé une fois au démarrage (startup()).
async function initLocalDB() {
  window._sb = new LocalDB();
  await idbOpen(); // lève une erreur explicite ici si IndexedDB est indisponible
  return window._sb;
}
...
// Marqueur "migration IndexedDB déjà effectuée sur ce navigateur" (Phase 1, v2026.07.13) —
// distinct de LS_CFG/LS_ANON_KEY (conservés pour la migration ponctuelle et une reconnexion
// manuelle éventuelle, mais plus utilisés pour décider du mode de démarrage courant).
const LS_IDB_MIGRATED = 'discotheque_idb_migrated';
window._sb = null; // pointera vers le shim LocalDB (IndexedDB) après initLocalDB()

// Charge la collection depuis window._sb (le shim LocalDB/IndexedDB, déjà initialisé par
// initLocalDB() au démarrage) — remplace l'ancienne logique inline de connectSupabase() pour
// le cas normal (pas de migration à faire). Réutilise loadFromSupabase()/loadLastfmFromSupabase()/
// loadAlbumTracks()/loadMusicBeeTracks() SANS AUCUNE MODIFICATION : ces fonctions croient
// toujours parler à Supabase, elles lisent en réalité IndexedDB via le shim.
...
// temporairement window._sb dessus — tout le code de chargement existant (loadFromSupabase(),
...
async function migrateSupabaseToIndexedDB(anonKey) {
  ...
  const realSb = supabase.createClient(SUPABASE_URL, anonKey);
  window._sb = realSb;
  try {
    const loaded = await loadFromSupabase();
    await loadLastfmFromSupabase();
    ...
    renderAlbums(); renderTracks(); updateNavBadges();
    setSaveIndicator('saved');
    toast(loaded ? '✓ Migration IndexedDB terminée — Supabase ne sera plus interrogé automatiquement' : '✓ Migration terminée (collection vide côté Supabase)');
    return loaded;
  } catch(e) {
    window._sb = localSb;
    console.error('Migration IndexedDB échouée', e);
    toast('Erreur migration IndexedDB : ' + (e.message || e), 'error');
    throw e;
  }
}

// Conservée pour le bouton "Connecter →" de l'écran de configuration (index.html) : sert
// désormais à (re)déclencher une migration manuelle depuis Supabase (ex: 1er lancement sur un
// nouveau navigateur/appareil qui n'a pas encore d'IndexedDB local) plutôt qu'à ouvrir une
// connexion permanente — cf. migration-indexeddb.md, Phase 1.
async function connectSupabase(anonKey) {
  ...
  try {
    ...
    localStorage.removeItem(LS_IDB_MIGRATED); // force une (re)migration avec cette clé
    await migrateSupabaseToIndexedDB(key);
...
// ⚠️ DEAD CODE depuis v2026.07.13 (Phase 1 migration IndexedDB) : plus aucun appelant — le
// nouveau bootstrap (startup()) appelle loadLocalCollection() à la place. Conservée pour
// référence/diff le temps de valider la migration en conditions réelles ; à supprimer en
// Phase 3 si tout se confirme stable.
...
(async function startup() {
  // Phase 1 migration IndexedDB (v2026.07.13, cf. migration-indexeddb.md) : IndexedDB (via le
  // shim LocalDB) est désormais la source de vérité systématique — plus de réseau Supabase à
  // chaque ouverture. Supabase n'est contacté qu'UNE SEULE FOIS par navigateur, pour rapatrier
  // les données existantes, si des identifiants d'une session antérieure sont trouvés ET
  // qu'aucune migration n'a encore eu lieu ici (LS_IDB_MIGRATED absent).
  try {
    await initLocalDB();
  } catch(e) {
    // IndexedDB indisponible (navigateur en mode privé très restrictif, quota système à 0...)
    // — cas limite non géré en Phase 1, l'app reste utilisable en mémoire pour la session mais
    // rien ne sera persisté. Signalé clairement plutôt qu'un échec silencieux.
    console.error('IndexedDB indisponible :', e);
    toast('⚠️ IndexedDB indisponible — rien ne pourra être sauvegardé cette session', 'error');
    loadDemo();
    _dataReady = true;
    renderAlbums(); renderTracks(); updateNavBadges();
    return;
  ...
  const alreadyMigrated = localStorage.getItem(LS_IDB_MIGRATED);
  const savedCfg = localStorage.getItem(LS_CFG);
  const savedKey = localStorage.getItem(LS_ANON_KEY);

  if (!alreadyMigrated && savedCfg === 'supabase' && savedKey) {
    try {
      await migrateSupabaseToIndexedDB(savedKey);
    } catch(e) {
      // Réseau indisponible au moment de la migration : ne PAS marquer comme migré, on
      // retentera au prochain lancement — en attendant, IndexedDB est vide, rien à charger.
      console.warn('Migration Supabase → IndexedDB reportée (réseau indisponible ?) :', e);
  ...
  if (!alreadyMigrated) {
    // Pas d'identifiants Supabase antérieurs : rien à migrer, premier lancement en local pur.
    localStorage.setItem(LS_IDB_MIGRATED, '1');
  }
  ...
  if (!loaded) {
    ...
    _dataReady = true;
    renderAlbums(); renderTracks(); updateNavBadges();
    if (!savedKey) document.getElementById('supabase-setup').style.display = 'flex';
  }
})();
